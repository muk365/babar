document.addEventListener('DOMContentLoaded', () => {
    ...
    function setupEventHandlers() {
        ...
        // ✅ NOUVEAU : GESTIONNAIRE POUR LES MESSAGES RAW (solution principale)
        const onRawMessage = (messageCloned, message) => {
            // Traitement spécial pour les USERNOTICE qui ne passent pas par l'événement standard
            if (message.command === 'USERNOTICE' && message.tags) {
                debugLog(`🔍 RAW USERNOTICE détecté: ${message.tags['msg-id']}`);
                
                // Créer un faux channel et appeler onUserNotice directement
                const fakeChannel = message.params[0] || '#unknown';
                onUserNotice(fakeChannel, message.tags, message.params[1] || '', false);
            }
            
            // Log pour debug des messages bruts
            if (message.command === 'USERNOTICE' || (message.tags && message.tags.bits)) {
                debugLog(`RAW: ${message.command} - Tags: ${JSON.stringify(message.tags)}`);
            }
        };
        ...
        // ✅ NOUVEAU : Gestionnaire des messages bruts (crucial pour capturer tous les USERNOTICE)
        twitchClient.on('raw_message', onRawMessage);

        // ✅ GESTIONNAIRES POUR DIAGNOSTIQUER LES ÉVÉNEMENTS MANQUÉS
        ...
        // ✅ NOUVEAU : Gestionnaire pour les événements de resub spécifiques
        twitchClient.on('resub', (channel, username, months, message, userstate, methods) => {
            debugLog(`🔔 RESUB EVENT: ${username} - ${months} mois`);
            // Backup au cas où l'événement USERNOTICE ne fonctionne pas
            ...
            if (value > 0) {
                updateGlobalTotal(value, `Resub de ${username}`);
            }
        });
        ...
        eventHandlers.onRawMessage = onRawMessage;

        debugLog("✅ Gestionnaires d'événements configurés avec détection RAW améliorée");
    }
